...
public class RelatorioMB {
    ...
    public void gerarRelatorio(ActionEvent event) {
        ...
        try {
            administradorBO.gerarRelatorio(this.tipoRelatorio, dataInicial, dataFinal);
            fileDownloadView();
    ...
    public void fileDownloadView() {
        InputStream stream;
        if (this.tipoRelatorio.equals("1")) {
        ...
        } else if (this.tipoRelatorio.equals("2")) {
    ...
    public StreamedContent getFile() {
        if (this.tipoRelatorio.equals("1")) {
            this.stream = FacesContext.getCurrentInstance().getExternalContext().getResourceAsStream("/resources/relatorios/RelatorioSemanal.csv");
            return new DefaultStreamedContent(stream, "application/csv", "downloaded_RelatorioSemanal.csv");
        } else if (this.tipoRelatorio.equals("2")) {
            this.stream = FacesContext.getCurrentInstance().getExternalContext().getResourceAsStream("/resources/relatorios/RelatorioMensal.csv");
            return new DefaultStreamedContent(stream, "application/csv", "downloaded_RelatorioMensal.csv");
        } else {
            this.stream = FacesContext.getCurrentInstance().getExternalContext().getResourceAsStream("/resources/relatorios/RelatorioAnual.csv");
